...
public class ChatService : IChatService
{
    ...
    public async Task<ApiResponse<List<Chat>>> GetChatsAsync(int page = 1, int limit = 20)
    {
        ...
        if (response == null)
        {
            return new ApiResponse<List<Chat>>(null, "Ошибка подключения к серверу", null);
        }
        ...
        return new ApiResponse<List<Chat>>(false, json, null);
    }
    ...
    public async Task<ApiResponse<DetailChatInfo>> GetChatByIdAsync(int chatId, int page = 1, int limit = 50)
    {
        ...
        if (response == null)
        {
            return new ApiResponse<DetailChatInfo>(null, "Ошибка подключения к серверу", null);
        }
        ...
        return new ApiResponse<DetailChatInfo>(false, json, null);
    }
    ...
    public async Task<ApiResponse<int>> CreateChatAsync(string type, string? name, List<int> userIds)
    {
        ...
        if (response == null)
        {
            return new ApiResponse<int>(null, "Ошибка подключения к серверу", 0);
        }
        ...
        return new ApiResponse<int>(false, json, 0);
    }
    ...
    public async Task<ApiResponse<bool>> AddMemberAsync(int chatId, int userId)
    {
        ...
        if (response == null)
        {
            return new ApiResponse<bool>(null, "Ошибка подключения к серверу", false);
        }
    ...
    public async Task<ApiResponse<Message>> SendMessageAsync(int chatId, MessageSendModel message)
    {
        ...
        if (response == null)
        {
            return new ApiResponse<Message>(null, "Ошибка подключения к серверу", null);
        }
        ...
        return new ApiResponse<Message>(false, json, null);
    }
    ...
    public async Task<ApiResponse<nearby.Models.Messages>> GetMessagesAsync(int chatId, int page = 1, int limit = 50)
    {
        ...
        if (response == null)
        {
            return new ApiResponse<nearby.Models.Messages>(null, "Ошибка подключения к серверу", null);
        }
        ...
        return new ApiResponse<nearby.Models.Messages>(false, json, null);
    }
    ...
    public async Task<ApiResponse<Message>> EditMessageAsync(int messageId, string newContent)
    {
        ...
        if (response.IsSuccessStatusCode)
        {
            // API возвращает только success, поэтому вернём минимальное сообщение
            var msg = new Message
            {
                Id = messageId,
                Content = newContent
