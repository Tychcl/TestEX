...
/**
 * Base class that represents a row from the 'eventInfo' table.
 *
 * Чемпионаты
 *
 * @package    propel.generator.Models.Base
 */
abstract class Eventinfo implements ActiveRecordInterface
{
    /**
     * TableMap class name
     *
     * @var string
     */
    public const TABLE_MAP = '\\Models\\Map\\EventinfoTableMap';
    ...
    /**
     * Initializes internal state of Models\Base\Eventinfo object.
     */
    ...
    /**
     * Compares this with another <code>Eventinfo</code> instance.  If
     * <code>obj</code> is an instance of <code>Eventinfo</code>, delegates to
     * <code>equals(Eventinfo)</code>.  Otherwise, returns <code>false</code>.
     *
    ...
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[EventinfoTableMap::COL_ID] = true;
        }
    ...
    public function setName($v)
    {
        ...
        if ($this->name !== $v) {
            $this->name = $v;
            $this->modifiedColumns[EventinfoTableMap::COL_NAME] = true;
        }
    ...
    public function setStart($v)
    {
        ...
        if ($this->start !== null || $dt !== null) {
            if ($this->start === null || $dt === null || $dt->format("Y-m-d") !== $this->start->format("Y-m-d")) {
                ...
                $this->modifiedColumns[EventinfoTableMap::COL_START] = true;
            }
    ...
    public function setEnd($v)
    {
        ...
        if ($this->end !== null || $dt !== null) {
            if ($this->end === null || $dt === null || $dt->format("Y-m-d") !== $this->end->format("Y-m-d")) {
                ...
                $this->modifiedColumns[EventinfoTableMap::COL_END] = true;
            }
        } // if either are not null
    ...
    public function setLevel($v)
    {
        ...
        if ($this->level !== $v) {
            $this->level = $v;
            $this->modifiedColumns[EventinfoTableMap::COL_LEVEL] = true;
        }

        if ($this->aEventlevel !== null && $this->aEventlevel->getId() !== $v) {
            $this->aEventlevel = null;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : EventinfoTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : EventinfoTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : EventinfoTableMap::translateFieldName('Start', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->start = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : EventinfoTableMap::translateFieldName('End', TableMap::TYPE_PHPNAME, $indexType)];
            if ($col === '0000-00-00') {
                $col = null;
            }
            $this->end = (null !== $col) ? PropelDateTime::newInstance($col, null, 'DateTime') : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 4 + $startcol : EventinfoTableMap::translateFieldName('Level', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 5; // 5 = EventinfoTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\Models\\Eventinfo'), 0, $e);
        }
    }
    ...
    {
        if ($this->aEventlevel !== null && $this->level !== $this->aEventlevel->getId()) {
            $this->aEventlevel = null;
        }
    }
    ...
    public function reload(bool $deep = false, ?ConnectionInterface $con = null): void
    {
        ...
        $dataFetcher = ChildEventinfoQuery::create(null, $this->buildPkeyCriteria())->setFormatter(ModelCriteria::FORMAT_STATEMENT)->find($con);
     ...
     * @return void
     * @throws \Propel\Runtime\Exception\PropelException
     * @see Eventinfo::setDeleted()
     * @see Eventinfo::isDeleted()
     */
    public function delete(?ConnectionInterface $con = null): void
    {
        ...
        $con->transaction(function () use ($con) {
            $deleteQuery = ChildEventinfoQuery::create()
    ...
    public function save(?ConnectionInterface $con = null): int
    {
        ...
        return $con->transaction(function () use ($con) {
            ...
            if ($ret) {
                ...
                $this->postSave($con);
                EventinfoTableMap::addInstanceToPool($this);
            } else {
                $affectedRows = 0;
            }
    ...
    protected function doSave(ConnectionInterface $con): int
    {
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->aEventlevel !== null) {
                if ($this->aEventlevel->isModified() || $this->aEventlevel->isNew()) {
                    $affectedRows += $this->aEventlevel->save($con);
                }
                $this->setEventlevel($this->aEventlevel);
            }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        $this->modifiedColumns[EventinfoTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . EventinfoTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(EventinfoTableMap::COL_ID)) {
            $modifiedColumns[':p' . $index++]  = 'id';
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_START)) {
            $modifiedColumns[':p' . $index++]  = 'start';
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_END)) {
            $modifiedColumns[':p' . $index++]  = 'end';
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_LEVEL)) {
            $modifiedColumns[':p' . $index++]  = 'level';
        }

        $sql = sprintf(
            'INSERT INTO eventInfo (%s) VALUES (%s)',
    ...
    {
        $pos = EventinfoTableMap::translateFieldName($name, $type, TableMap::TYPE_NUM);
    ...
    {
        if (isset($alreadyDumpedObjects['Eventinfo'][$this->hashCode()])) {
            return ['*RECURSION*'];
        }
        $alreadyDumpedObjects['Eventinfo'][$this->hashCode()] = true;
        $keys = EventinfoTableMap::getFieldNames($keyType);
        $result = [
            $keys[0] => $this->getId(),
            $keys[1] => $this->getName(),
        ...
        if ($includeForeignObjects) {
            if (null !== $this->aEventlevel) {
                ...
                $result[$key] = $this->aEventlevel->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
            }
    ...
    {
        $criteria = new Criteria(EventinfoTableMap::DATABASE_NAME);

        if ($this->isColumnModified(EventinfoTableMap::COL_ID)) {
            $criteria->add(EventinfoTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_NAME)) {
            $criteria->add(EventinfoTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_START)) {
            $criteria->add(EventinfoTableMap::COL_START, $this->start);
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_END)) {
            $criteria->add(EventinfoTableMap::COL_END, $this->end);
        }
        if ($this->isColumnModified(EventinfoTableMap::COL_LEVEL)) {
            $criteria->add(EventinfoTableMap::COL_LEVEL, $this->level);
        }
    ...
    {
        $criteria = ChildEventinfoQuery::create();
        $criteria->add(EventinfoTableMap::COL_ID, $this->id);
     ...
     * objects.
     *
     * @param object $copyObj An object of \Models\Eventinfo (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Models\Eventinfo Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    ...
    /**
     * Declares an association between this object and a ChildEventlevel object.
     *
     * @param ChildEventlevel|null $v
     * @return $this The current object (for fluent API support)
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function setEventlevel(ChildEventlevel $v = null)
    {
        if ($v === null) {
            $this->setLevel(NULL);
        } else {
        ...
        // If this object has already been added to the ChildEventlevel object, it will not be re-added.
        if ($v !== null) {
            $v->addEventinfo($this);
        }
    ...
    /**
     * Get the associated ChildEventlevel object
     *
     * @param ConnectionInterface $con Optional Connection object.
     * @return ChildEventlevel|null The associated ChildEventlevel object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getEventlevel(?ConnectionInterface $con = null)
    {
        if ($this->aEventlevel === null && ($this->level != 0)) {
            $this->aEventlevel = ChildEventlevelQuery::create()->findPk($this->level, $con);
            /* The following can be used additionally to
                guarantee the related object contains a reference
                to this object.  This level of coupling may, however, be
                undesirable since it could result in an only partially populated collection
                in the referenced object.
                $this->aEventlevel->addEventinfos($this);
             */
        }

        return $this->aEventlevel;
    }
     ...
     * If this ChildEventinfo is new, it will return
    ...
    public function setEvents(Collection $events, ?ConnectionInterface $con = null)
    {
        ...
        foreach ($eventsToDelete as $eventRemoved) {
            $eventRemoved->setEventinfo(null);
        }
     ...
     * Otherwise if this Eventinfo is new, it will return
     * an empty collection; or if this Eventinfo has previously
     * been saved, it will retrieve related Events from storage.
     *
     * This method is protected by default in order to keep the public
     * api reasonable.  You can provide public methods for those you
     * actually need in Eventinfo.
     *
     * @param Criteria $criteria optional Criteria object to narrow the query
     * @param ConnectionInterface $con optional connection object
     ...
     * @phpstan-return ObjectCollection&\Traversable<ChildEvent}> List of ChildEvent objects
     */
    public function getEventsJoinTeacher(?Criteria $criteria = null, ?ConnectionInterface $con = null, $joinBehavior = Criteria::LEFT_JOIN)
    {
        $query = ChildEventQuery::create(null, $criteria);
        $query->joinWith('Teacher', $joinBehavior);
    ...
    {
        if (null !== $this->aEventlevel) {
            $this->aEventlevel->removeEventinfo($this);
        }
        $this->id = null;
        $this->name = null;
        $this->start = null;
    ...
    {
        return (string) $this->exportTo(EventinfoTableMap::DEFAULT_STRING_FORMAT);
    }
