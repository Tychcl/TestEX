...
abstract class Student implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Models\\Map\\StudentTableMap';
    ...
    /**
     * Initializes internal state of Models\Base\Student object.
     */
    ...
    public function setSurname($v)
    {
        ...
        if ($this->surname !== $v) {
            $this->surname = $v;
            $this->modifiedColumns[StudentTableMap::COL_SURNAME] = true;
        }
    ...
    public function setMidname($v)
    {
        ...
        if ($this->midname !== $v) {
            $this->midname = $v;
            $this->modifiedColumns[StudentTableMap::COL_MIDNAME] = true;
        }
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : StudentTableMap::translateFieldName('Surname', TableMap::TYPE_PHPNAME, $indexType)];
            $this->surname = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : StudentTableMap::translateFieldName('Name', TableMap::TYPE_PHPNAME, $indexType)];
            $this->name = (null !== $col) ? (string) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 3 + $startcol : StudentTableMap::translateFieldName('Midname', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Models\\Student'), 0, $e);
        }
    }
    ...
    protected function doInsert(ConnectionInterface $con): void
    {
        ...
        if ($this->isColumnModified(StudentTableMap::COL_SURNAME)) {
            $modifiedColumns[':p' . $index++]  = 'surname';
        }
        if ($this->isColumnModified(StudentTableMap::COL_NAME)) {
            $modifiedColumns[':p' . $index++]  = 'name';
        }
        if ($this->isColumnModified(StudentTableMap::COL_MIDNAME)) {
    ...
    public function buildCriteria(): Criteria
    {
        ...
        if ($this->isColumnModified(StudentTableMap::COL_SURNAME)) {
            $criteria->add(StudentTableMap::COL_SURNAME, $this->surname);
        }
        if ($this->isColumnModified(StudentTableMap::COL_NAME)) {
            $criteria->add(StudentTableMap::COL_NAME, $this->name);
        }
        if ($this->isColumnModified(StudentTableMap::COL_MIDNAME)) {
            $criteria->add(StudentTableMap::COL_MIDNAME, $this->midname);
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Models\Student (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Models\Student Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
