...
/**
 * Base class that represents a query for the `userRole` table.
 *
 ...
 *
 * @method     ChildUserroleQuery leftJoinTeacher($relationAlias = null) Adds a LEFT JOIN clause to the query using the Teacher relation
 * @method     ChildUserroleQuery rightJoinTeacher($relationAlias = null) Adds a RIGHT JOIN clause to the query using the Teacher relation
 * @method     ChildUserroleQuery innerJoinTeacher($relationAlias = null) Adds a INNER JOIN clause to the query using the Teacher relation
 *
 * @method     ChildUserroleQuery joinWithTeacher($joinType = Criteria::INNER_JOIN) Adds a join clause and with to the query using the Teacher relation
 *
 * @method     ChildUserroleQuery leftJoinWithTeacher() Adds a LEFT JOIN clause and with to the query using the Teacher relation
 * @method     ChildUserroleQuery rightJoinWithTeacher() Adds a RIGHT JOIN clause and with to the query using the Teacher relation
 * @method     ChildUserroleQuery innerJoinWithTeacher() Adds a INNER JOIN clause and with to the query using the Teacher relation
 *
 ...
 * @method     ChildUserrole[]|Collection findByName(string|array<string> $name) Return ChildUserrole objects filtered by the name column
 * @psalm-method Collection&\Traversable<ChildUserrole> findByName(string|array<string> $name) Return ChildUserrole objects filtered by the name column
 *
...
abstract class UserroleQuery extends ModelCriteria
{
    ...
    /**
     * Initializes internal state of \Models\Base\UserroleQuery object.
     *
     * @param string $dbName The database name
     * @param string $modelName The phpName of a model, e.g. 'Book'
     * @param string $modelAlias The alias for the model in this query, e.g. 'b'
     */
    public function __construct($dbName = 'default', $modelName = '\\Models\\Userrole', $modelAlias = null)
    {
    ...
    {
        $sql = 'SELECT id, name FROM userRole WHERE id = :p0';
        try {
     ...
     *
     * @param \Models\Teacher|ObjectCollection $teacher the related object to use as filter
     * @param string|null $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return $this The current query, for fluid interface
     */
    public function filterByTeacher($teacher, ?string $comparison = null)
    {
        if ($teacher instanceof \Models\Teacher) {
            $this
                ->addUsingAlias(UserroleTableMap::COL_ID, $teacher->getRoleid(), $comparison);
