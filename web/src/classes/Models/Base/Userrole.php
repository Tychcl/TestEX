...
use Models\Map\UserroleTableMap;
...
/**
 * Base class that represents a row from the 'userRole' table.
 *
 *
 *
...
abstract class Userrole implements ActiveRecordInterface
{
     ...
     * @var string
     */
    public const TABLE_MAP = '\\Models\\Map\\UserroleTableMap';
    ...
    /**
     * @var        ObjectCollection|ChildTeacher[] Collection to store aggregation of ChildTeacher objects.
     * @phpstan-var ObjectCollection&\Traversable<ChildTeacher> Collection to store aggregation of ChildTeacher objects.
     */
     ...
     * @var ObjectCollection|ChildTeacher[]
     * @phpstan-var ObjectCollection&\Traversable<ChildTeacher>
     */
    protected $teachersScheduledForDeletion = null;

    /**
     * Initializes internal state of Models\Base\Userrole object.
     */
    ...
    public function hydrate(array $row, int $startcol = 0, bool $rehydrate = false, string $indexType = TableMap::TYPE_NUM): int
    {
        try {
            ...
            throw new PropelException(sprintf('Error populating %s object', '\\Models\\Userrole'), 0, $e);
        }
    }
    ...
    public function toArray(string $keyType = TableMap::TYPE_PHPNAME, bool $includeLazyLoadColumns = true, array $alreadyDumpedObjects = [], bool $includeForeignObjects = false): array
    {
        ...
        if ($includeForeignObjects) {
            if (null !== $this->collTeachers) {
                ...
                $result[$key] = $this->collTeachers->toArray(null, false, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
            }
        }
     ...
     * objects.
     *
     * @param object $copyObj An object of \Models\Userrole (or compatible) type.
     ...
     *
     * @param bool $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Models\Userrole Clone of current object.
     * @throws \Propel\Runtime\Exception\PropelException
     */
     ...
     * @return void
     */
    public function resetPartialTeachers($v = true): void
    {
     ...
     * @return void
     */
    public function initTeachers(bool $overrideExisting = true): void
    {
        if (null !== $this->collTeachers && !$overrideExisting) {
            return;
        }

        $collectionClassName = TeacherTableMap::getTableMap()->getCollectionClassName();

        $this->collTeachers = new $collectionClassName;
        $this->collTeachers->setModel('\Models\Teacher');
    }

    /**
     * Gets an array of ChildTeacher objects which contain a foreign key that references this object.
     *
     ...
     * @return ObjectCollection|ChildTeacher[] List of ChildTeacher objects
     * @phpstan-return ObjectCollection&\Traversable<ChildTeacher> List of ChildTeacher objects
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function getTeachers(?Criteria $criteria = null, ?ConnectionInterface $con = null)
    {
        ...
        if (null === $this->collTeachers || null !== $criteria || $partial) {
            if ($this->isNew()) {
                ...
                if (null === $this->collTeachers) {
                    ...
                    $collTeachers = new $collectionClassName;
                    $collTeachers->setModel('\Models\Teacher');

                    return $collTeachers;
                }
            } else {
                $collTeachers = ChildTeacherQuery::create(null, $criteria)
    ...
    /**
     * Sets a collection of ChildTeacher objects related by a one-to-many relationship
     * to the current object.
     * It will also schedule objects for deletion based on a diff between old objects (aka persisted)
     * and new objects from the given Propel collection.
     *
     * @param Collection $teachers A Propel collection.
     * @param ConnectionInterface $con Optional connection object
     * @return $this The current object (for fluent API support)
     */
    public function setTeachers(Collection $teachers, ?ConnectionInterface $con = null)
    {
        /** @var ChildTeacher[] $teachersToDelete */
        $teachersToDelete = $this->getTeachers(new Criteria(), $con)->diff($teachers);


        $this->teachersScheduledForDeletion = $teachersToDelete;

        foreach ($teachersToDelete as $teacherRemoved) {
            $teacherRemoved->setUserrole(null);
        }
     ...
     * @throws \Propel\Runtime\Exception\PropelException
     */
    public function countTeachers(?Criteria $criteria = null, bool $distinct = false, ?ConnectionInterface $con = null): int
    {
        $partial = $this->collTeachersPartial && !$this->isNew();
        if (null === $this->collTeachers || null !== $criteria || $partial) {
            ...
            $query = ChildTeacherQuery::create(null, $criteria);
            if ($distinct) {
                $query->distinct();
            }
    ...
    /**
     * Method called to associate a ChildTeacher object to this object
     * through the ChildTeacher foreign key attribute.
     *
     * @param ChildTeacher $l ChildTeacher
     * @return $this The current object (for fluent API support)
     */
    public function addTeacher(ChildTeacher $l)
    {
        if ($this->collTeachers === null) {
            $this->initTeachers();
            $this->collTeachersPartial = true;
        ...
        if (!$this->collTeachers->contains($l)) {
            $this->doAddTeacher($l);

            if ($this->teachersScheduledForDeletion and $this->teachersScheduledForDeletion->contains($l)) {
                $this->teachersScheduledForDeletion->remove($this->teachersScheduledForDeletion->search($l));
            }
        }
    ...
    /**
     * @param ChildTeacher $teacher The ChildTeacher object to add.
     */
    protected function doAddTeacher(ChildTeacher $teacher): void
    {
        $this->collTeachers[]= $teacher;
        $teacher->setUserrole($this);
    }

    /**
     * @param ChildTeacher $teacher The ChildTeacher object to remove.
     * @return $this The current object (for fluent API support)
     */
    public function removeTeacher(ChildTeacher $teacher)
    {
        if ($this->getTeachers()->contains($teacher)) {
            $pos = $this->collTeachers->search($teacher);
            $this->collTeachers->remove($pos);
